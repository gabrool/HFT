"""Build the canonical decision_grid for one execution tape."""
...
def _default_output_grid(tape_root: str) -> Path:
...
class _DecisionGridWriters:
    def __init__(self, output_grid: Path, *, chunk_rows: int) -> None:
        self.output_grid = output_grid
        self.temp_root = output_grid.parent / f".{output_grid.name}.tmp"
        self.chunk_dir = self.temp_root / "chunks"
        self.arrays_dir = self.temp_root / "arrays"
    ...
    def publish(self) -> None:
        if self.output_grid.exists():
            if self.output_grid.is_dir():
                shutil.rmtree(self.output_grid)
            else:
                self.output_grid.unlink()
        self.temp_root.replace(self.output_grid)
...
def build_decision_grid_from_config(config: BuildDecisionGridConfig) -> dict[str, object]:
    ...
    output_grid = Path(config.output_grid) if config.output_grid is not None else _default_output_grid(config.tape_root)
    output_json = Path(config.output_json) if config.output_json is not None else _default_output_json(config.tape_root)
    if output_grid.exists() and not config.overwrite:
        raise FileExistsError(f"output_grid already exists: {output_grid}")
    ...
    writers = _DecisionGridWriters(output_grid, chunk_rows=config.chunk_rows)
    ...
    reason_counts_by_code: dict[int, int] = {}
    interval_stats = {
        "elapsed_since_prev_decision_us": _RunningStats(),
        "events_since_prev_decision": _RunningStats(),
        "l2_events_since_prev_decision": _RunningStats(),
        "trade_events_since_prev_decision": _RunningStats(),
    }
    try:
        for event_index in range(config.start_event_index, len(events)):
            ...
            reason_counts_by_code[int(fire.reason_code)] = reason_counts_by_code.get(int(fire.reason_code), 0) + 1
            if last_decision_event_index is not None:
                interval_stats["elapsed_since_prev_decision_us"].update(elapsed)
                interval_stats["events_since_prev_decision"].update(events_since)
                interval_stats["l2_events_since_prev_decision"].update(l2_since)
                interval_stats["trade_events_since_prev_decision"].update(trade_since)
            writers.append(
        ...
        validate_decision_grid_for_execution_tape(grid, tape, mode="full")
        write_decision_grid_manifest(writers.temp_root, metadata)
        arrays_owner.close()
        arrays_owner = None
        writers.cleanup_chunks()
        writers.publish()
        published = True
        grid = load_decision_grid(output_grid, mmap_mode=config.mmap_mode)
        reason_counts = {
            DECISION_REASON_CODE_NAMES.get(int(code), str(int(code))): int(count)
            for code, count in sorted(reason_counts_by_code.items())
        }
        summary = {
            "status": "ok",
            ...
            "decision_grid": decision_grid_summary(grid, path=str(output_grid)),
            "reason_counts": reason_counts,
            "interval_stats": {name: stats.as_dict() for name, stats in interval_stats.items()},
            "counters": {
