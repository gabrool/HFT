...
class BookState:
    ...
    def _liquidity_void(self,side:str)->float:
        sz=(self.current_bid_sz if side=="bid" else self.current_ask_sz)[:20]; px=(self.current_bid_px if side=="bid" else self.current_ask_px)[:20]; pos=sz[sz>0]
        if pos.size==0: return 0.0
        min_size=float(np.median(pos)); idx=np.where(sz>=min_size)[0]
        if idx.size<2: return 0.0
        return abs(_safe_bps_change(px[idx[0]],px[idx[-1]]))
    ...
    def fill_book_features(self, out: np.ndarray) -> np.ndarray:
        ...
        arr=np.asarray(out)
        if arr.ndim!=1 or arr.shape[0]!=FEATURE_COUNT: raise ValueError
        assigned=set(); now=self.last_local_ts_us; s=self.current_summary(); depth=max(s.total_depth_5bps_size,FLOAT_EPS); b1=max(self._depth_size_within_bps('bid',1.0),FLOAT_EPS); a1=max(self._depth_size_within_bps('ask',1.0),FLOAT_EPS)
        def setf(name,v): arr[feature_spec_by_name(name).index]=_finite(v); assigned.add(name)
        # Temporary zero because this requires trade timestamps; trade_state/engine must override or re-own before production.
        setf("time_since_trade_us",0.0); setf("spread_bps",s.spread_bps); setf("gap_b_bps",self._gap_b_bps()); setf("bsz1",self.current_bid_sz[0]); setf("asz1",self.current_ask_sz[0]); setf("micro_minus_mid_bps",s.micro_minus_mid_bps); setf("time_since_mid_change_us",now-self.last_mid_change_ts_us)
        setf("micro_ret_bps_200000us",_safe_bps_change(self._microprice_l1(),self._window_values('microprice',WINDOW_200MS_US)[0] if self._window_values('microprice',WINDOW_200MS_US).size else self._microprice_l1())); setf("micro_ret_bps_500000us",_safe_bps_change(self._microprice_l1(),self._window_values('microprice',WINDOW_500MS_US)[0] if self._window_values('microprice',WINDOW_500MS_US).size else self._microprice_l1())); setf("micro_ret_bps_1000000us",_safe_bps_change(self._microprice_l1(),self._window_values('microprice',WINDOW_1000MS_US)[0] if self._window_values('microprice',WINDOW_1000MS_US).size else self._microprice_l1()))
        ...
        setf("bid_depth_within_1bps",b1); setf("ask_depth_within_1bps",a1); setf("depth_imbalance_within_1bps",self._depth_imbalance_within_bps(1.0))
        ...
        setf("ask_l1_depletion_over_depth_200000us",self._rolling_sum('ask_l1_rem',WINDOW_200MS_US)/a1); setf("bid_l1_depletion_over_depth_500000us",self._rolling_sum('bid_l1_rem',WINDOW_500MS_US)/b1); setf("ask_l1_depletion_over_depth_500000us",self._rolling_sum('ask_l1_rem',WINDOW_500MS_US)/a1); setf("bid_l1_depletion_over_depth_1000000us",self._rolling_sum('bid_l1_rem',WINDOW_1000MS_US)/b1); setf("ask_l1_depletion_over_depth_1000000us",self._rolling_sum('ask_l1_rem',WINDOW_1000MS_US)/a1)
        ...
        setf("bid_l1_add_rate_over_depth_200000us",self._rolling_sum('bid_l1_add',WINDOW_200MS_US)/(0.2*b1)); setf("bid_l1_rem_rate_over_depth_200000us",self._rolling_sum('bid_l1_rem',WINDOW_200MS_US)/(0.2*b1)); setf("ask_l1_add_rate_over_depth_200000us",self._rolling_sum('ask_l1_add',WINDOW_200MS_US)/(0.2*a1)); setf("bid_l1_add_rate_over_depth_500000us",self._rolling_sum('bid_l1_add',WINDOW_500MS_US)/(0.5*b1)); setf("ask_l1_add_rate_over_depth_500000us",self._rolling_sum('ask_l1_add',WINDOW_500MS_US)/(0.5*a1)); setf("bid_l1_add_rate_over_depth_1000000us",self._rolling_sum('bid_l1_add',WINDOW_1000MS_US)/b1); setf("ask_l1_add_rate_over_depth_1000000us",self._rolling_sum('ask_l1_add',WINDOW_1000MS_US)/a1)
        setf("vwap_vs_mid_bps_200000us",0.0); setf("vwap_vs_mid_bps_500000us",0.0); setf("return_std_bps_200000us",self._rolling_return_std_bps('mid',WINDOW_200MS_US)); setf("regime_volume_ewma_500000us",self._rolling_mean('total_depth_5bps_notional',WINDOW_500MS_US)); setf("down_up_vol_imbalance_500000us",0.0); setf("max_abs_return_bps_500000us",self._rolling_max_abs('micro_minus_mid_bps',WINDOW_500MS_US)); setf("down_up_vol_imbalance_1000000us",0.0); setf("regime_volume_ewma_3000000us",self._rolling_mean('total_depth_5bps_notional',WINDOW_3000MS_US)); setf("down_up_vol_imbalance_3000000us",0.0)
