...
def train_and_evaluate():
    ...
    ob_files = sorted(glob.glob(os.path.join(DATA_ROOT, "OB", "BTCUSDT_OB_*.zip")))
    th_files = sorted(glob.glob(os.path.join(DATA_ROOT, "TH", "BTCUSDT_TH_*.zip")))
    if len(ob_files) != len(th_files):
        print(f"Warning: OB files: {len(ob_files)} vs TH files: {len(th_files)} — pairing will zip shortest lists.")
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in pbar:
            ...
            if is_ssl_pretrain:
                ...
                y = (y_ret > 0).float()
                mask = (y_ret.abs() > TAU)
                bce_loss = F.binary_cross_entropy_with_logits(dir_pred_logits[mask], y[mask], reduction='mean')
            ...
            if is_ssl_pretrain:
                ...
                y = (y_ret > 0).float()
                mask = (y_ret.abs() > TAU)
                bce_loss2 = F.binary_cross_entropy_with_logits(dir_pred_logits2[mask], y[mask], reduction='mean')
