...
def test_stage4_accepts_train_full_default_streaming(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda out_root: {
            "ds_train_list": [FakeDataset(10), FakeDataset(15)],
            "ds_val": FakeDataset(8),
            "ds_test": None,
...
def test_stage4_rejects_stale_stage3_decision_metadata(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        linear_offline,
        "build_linear_cmssl_splits_from_out_root",
        lambda out_root: {"ds_train_list": [FakeDataset(10)], "ds_val": FakeDataset(8), "ds_test": None, "has_cmssl_test": False},
    )
    monkeypatch.setattr(linear_offline, "load_stage2_extractor_bundle", lambda **kwargs: (object(), {"payload_path": "stage2.json"}))
...
def test_stage3_records_train_full_fit_metadata(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(linear_offline, "load_stage2_extractor_bundle", lambda **kwargs: (object(), {"payload_path": "stage2.json"}))
    monkeypatch.setattr(linear_offline, "_fit_preprocess_from_stream", lambda *args, **kwargs: bundle)
