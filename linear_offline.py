...
def run_stage4_training(*, linear_out_dir: Path, extractor_name: str, preprocess_name: str, device: torch.device) -> Dict[str, Any]:  # type: ignore[override]
    ...
    payload={"stage":"stage4","status":"ok","schema":LINEAR_STAGE4_SCHEMA,"stage4_config":cfg,"best_direction_alpha":float(best_direction_result['direction_alpha']),"best_mag_alpha":float(best_magnitude_result['mag_alpha']),"best_move_alpha":float(best_move_result['move_alpha']),"direction_candidate_summaries":direction_summaries,"magnitude_candidate_summaries":magnitude_summaries,"move_candidate_summaries":move_candidate_summaries,"move_head_enabled":True,"move_target_schema":"abs_return_exceeds_side_low_threshold_v1","move_training_rows":"all_decision_rows","move_weighting":LINEAR_STAGE4_MOVE_WEIGHTING,"move_alpha_grid":[float(a) for a in LINEAR_STAGE4_MOVE_ALPHA_VALUES],"final_edge_schema":"p_move_times_conditional_side_edge_v1","best_model_path":str(model_path),"val_metrics":_jsonable_metrics(best_val_metrics),"test_metrics":_jsonable_metrics(test_metrics) if test_metrics is not None else None,"stage4_summary_metrics":stage4_summary_metrics}
