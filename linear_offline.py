...
def _mag_primary_metrics_1s(metrics: dict) -> dict[str, float]:
    return {
        ...
        "zero_pred": _metric_at_primary_horizon_with_fallback(metrics, "zero_row_mean_pred_mag_bps", None),
...
def add_side_cond_log_magnitude_metrics(metrics: Dict[str, Any], *, y: np.ndarray, pred: Dict[str, np.ndarray], scale_up_bps: np.ndarray, scale_down_bps: np.ndarray) -> None:
    ...
    for h in range(y.shape[1]):
        ...
        metrics.setdefault("zero_row_mean_pred_mag_bps",[]).append(float(np.mean(0.5*(mag_up_bps[zero_rows,h]+mag_down_bps[zero_rows,h]))) if np.any(zero_rows) else float('nan'))
...
def add_move_head_metrics(metrics: dict, *, y: np.ndarray, pred: dict, stats: dict) -> None:
    ...
    move_auc=[]; move_bal_acc=[]; move_bce=[]; move_pos_frac_true=[]; move_pos_frac_pred_p50=[]
    move_prob_mean=[]; move_prob_p10=[]; move_prob_p50=[]; move_prob_p90=[]
    move_prob_mean_zero_rows=[]; move_prob_p50_zero_rows=[]; move_prob_p90_zero_rows=[]
    move_prob_mean_nonmove_rows=[]; move_prob_p90_nonmove_rows=[]
    move_prob_mean_move_rows=[]; move_prob_p10_move_rows=[]; move_prob_p50_move_rows=[]
    cond_edge_spearman_all=[]; cond_edge_spearman_kept=[]; cond_edge_spearman_q50plus=[]; cond_edge_spearman_q85plus=[]; cond_edge_bal_sign_acc_q50plus=[]
    gated_edge_spearman_all=[]; gated_edge_spearman_kept=[]; gated_edge_spearman_q50plus=[]; gated_edge_spearman_q85plus=[]; gated_edge_bal_sign_acc_q50plus=[]
    for h in range(y.shape[1]):
        target = np.asarray(move_target[:, h], dtype=np.int64)
        p = np.asarray(pred["p_move"][:, h], dtype=np.float32)
        eps = 1e-6
        p_clip = np.clip(p, eps, 1.0 - eps)
        bce = -np.mean(target * np.log(p_clip) + (1 - target) * np.log(1 - p_clip))
        auc = _binary_auc_np(p, target.astype(bool))
        pred_label = p >= 0.5
        ...
        true = y[:, h]
        kept = keep_signed[:, h]
        move_auc.append(auc); move_bal_acc.append(bal); move_bce.append(float(bce)); move_pos_frac_true.append(float(np.mean(target))); move_pos_frac_pred_p50.append(float(np.mean(pred_label)))
        move_prob_mean.append(float(np.mean(p))); move_prob_p10.append(float(np.percentile(p,10))); move_prob_p50.append(float(np.percentile(p,50))); move_prob_p90.append(float(np.percentile(p,90)))
        move_prob_mean_zero_rows.append(float(np.mean(p[zero_rows])) if zero_rows.any() else math.nan)
        move_prob_p50_zero_rows.append(float(np.percentile(p[zero_rows],50)) if zero_rows.any() else math.nan)
        move_prob_p90_zero_rows.append(float(np.percentile(p[zero_rows],90)) if zero_rows.any() else math.nan)
        move_prob_mean_nonmove_rows.append(float(np.mean(p[nonmove_rows])) if nonmove_rows.any() else math.nan)
        move_prob_p90_nonmove_rows.append(float(np.percentile(p[nonmove_rows],90)) if nonmove_rows.any() else math.nan)
        move_prob_mean_move_rows.append(float(np.mean(p[move_rows])) if move_rows.any() else math.nan)
        move_prob_p10_move_rows.append(float(np.percentile(p[move_rows],10)) if move_rows.any() else math.nan)
        move_prob_p50_move_rows.append(float(np.percentile(p[move_rows],50)) if move_rows.any() else math.nan)
        cond_edge_spearman_all.append(_safe_spearman_np(cond_edge,true)); cond_edge_spearman_kept.append(_safe_spearman_np(cond_edge[kept],true[kept]) if kept.any() else math.nan)
        cond_edge_spearman_q50plus.append(math.nan); cond_edge_spearman_q85plus.append(math.nan); cond_edge_bal_sign_acc_q50plus.append(math.nan)
        gated_edge_spearman_all.append(_safe_spearman_np(edge,true)); gated_edge_spearman_kept.append(_safe_spearman_np(edge[kept],true[kept]) if kept.any() else math.nan)
        gated_edge_spearman_q50plus.append(math.nan); gated_edge_spearman_q85plus.append(math.nan); gated_edge_bal_sign_acc_q50plus.append(_balanced_acc_bool(edge[kept] >= 0.0, true[kept] > 0.0) if kept.any() else math.nan)
    metrics["move_auc"]=move_auc; metrics["move_bal_acc"]=move_bal_acc; metrics["move_bce"]=move_bce; metrics["move_pos_frac_true"]=move_pos_frac_true; metrics["move_pos_frac_pred_p50"]=move_pos_frac_pred_p50
    metrics["move_prob_mean"]=move_prob_mean; metrics["move_prob_p10"]=move_prob_p10; metrics["move_prob_p50"]=move_prob_p50; metrics["move_prob_p90"]=move_prob_p90
    metrics["move_prob_mean_zero_rows"]=move_prob_mean_zero_rows; metrics["move_prob_p50_zero_rows"]=move_prob_p50_zero_rows; metrics["move_prob_p90_zero_rows"]=move_prob_p90_zero_rows
    metrics["move_prob_mean_nonmove_rows"]=move_prob_mean_nonmove_rows; metrics["move_prob_p90_nonmove_rows"]=move_prob_p90_nonmove_rows
    metrics["move_prob_mean_move_rows"]=move_prob_mean_move_rows; metrics["move_prob_p10_move_rows"]=move_prob_p10_move_rows; metrics["move_prob_p50_move_rows"]=move_prob_p50_move_rows
    metrics["cond_edge_spearman_all"]=cond_edge_spearman_all; metrics["cond_edge_spearman_kept"]=cond_edge_spearman_kept; metrics["cond_edge_spearman_q50plus"]=cond_edge_spearman_q50plus; metrics["cond_edge_spearman_q85plus"]=cond_edge_spearman_q85plus; metrics["cond_edge_bal_sign_acc_q50plus"]=cond_edge_bal_sign_acc_q50plus
    metrics["gated_edge_spearman_all"]=gated_edge_spearman_all; metrics["gated_edge_spearman_kept"]=gated_edge_spearman_kept; metrics["gated_edge_spearman_q50plus"]=gated_edge_spearman_q50plus; metrics["gated_edge_spearman_q85plus"]=gated_edge_spearman_q85plus; metrics["gated_edge_bal_sign_acc_q50plus"]=gated_edge_bal_sign_acc_q50plus
...
def build_stage4_bundle_from_parts(*, config: Dict[str, Any], horizons_ms: list[int], direction_result: Dict[str, Any], magnitude_result: Dict[str, Any], move_result: Dict[str, Any]) -> LinearSklearnTakerBundle:
    ...
    fit_summary = {**direction_result.get("fit_summary", {}), **magnitude_result.get("fit_summary", {}), **move_result.get("fit_summary", {}), "direction_alpha": direction_alpha, "mag_alpha": mag_alpha, "move_alpha": move_alpha, "direction_fit_summary": dict(direction_result.get("fit_summary", {})), "magnitude_fit_summary": dict(magnitude_result.get("fit_summary", {})), "move_fit_summary": dict(move_result.get("fit_summary", {})), "combined_stage4_bundle": True}
...
def run_stage4_training(*, linear_out_dir: Path, extractor_name: str, preprocess_name: str, device: torch.device) -> Dict[str, Any]:  # type: ignore[override]
    ...
    for mb in magnitude_results:
        ...
        print(f"[linear-stage4-mag-candidate] mag_mode={mag_mode} mag_alpha={mag_alpha:g} log_huber_1s={m1['huber']:.6g} sp_1s={m1['spearman']:.6g} p50_ratio_1s={m1['p50_ratio']:.6g} p90_ratio_1s={m1['p90_ratio']:.6g} lift_1s={m1['lift']:.6g} edge_sp_1s={m1['edge_all']:.6g} selection_score={mag_score}", flush=True)
    ...
    for mv in move_results:
        ...
        edge_all=_metric_at_primary_horizon(mvm,'gated_edge_spearman_all'); edge_kept=_metric_at_primary_horizon(mvm,'gated_edge_spearman_kept'); move_auc=_metric_at_primary_horizon(mvm,'move_auc'); move_bce=_metric_at_primary_horizon(mvm,'move_bce')
        ...
        summary={"move_alpha": float(mv['move_alpha']), "move_auc_1s": float(_metric_at_primary_horizon(mvm,'move_auc')), "move_bal_1s": float(_metric_at_primary_horizon(mvm,'move_bal_acc')), "move_bce_1s": float(_metric_at_primary_horizon(mvm,'move_bce')), "move_pos_frac_true_1s": float(_metric_at_primary_horizon(mvm,'move_pos_frac_true')), "p_move_zero_1s": float(_metric_at_primary_horizon(mvm,'move_prob_mean_zero_rows')), "p_move_nonmove_1s": float(_metric_at_primary_horizon(mvm,'move_prob_mean_nonmove_rows')), "p_move_move_1s": float(_metric_at_primary_horizon(mvm,'move_prob_mean_move_rows')), "cond_edge_sp_all_1s": float(_metric_at_primary_horizon(mvm,'cond_edge_spearman_all')), "gated_edge_sp_all_1s": float(_metric_at_primary_horizon(mvm,'gated_edge_spearman_all')), "cond_edge_sp_kept_1s": float(_metric_at_primary_horizon(mvm,'cond_edge_spearman_kept')), "gated_edge_sp_kept_1s": float(_metric_at_primary_horizon(mvm,'gated_edge_spearman_kept')), "gated_edge_bal_sign_acc_q50plus_1s": float(_metric_at_primary_horizon(mvm,'gated_edge_bal_sign_acc_q50plus')), "edge_sp_all_1s": float(_metric_at_primary_horizon(mvm,'gated_edge_spearman_all')), "edge_sp_kept_1s": float(_metric_at_primary_horizon(mvm,'gated_edge_spearman_kept')), "edge_bal_sign_acc_q50plus_1s": float(_metric_at_primary_horizon(mvm,'gated_edge_bal_sign_acc_q50plus')), "selection_score": [float(x) for x in score]}
        move_candidate_summaries.append(summary)
        print(f"[linear-stage4-move-candidate] move_alpha={summary['move_alpha']:.6g} move_auc_1s={summary['move_auc_1s']:.6g} move_bal_1s={summary['move_bal_1s']:.6g} move_bce_1s={summary['move_bce_1s']:.6g} move_pos_frac_true_1s={summary['move_pos_frac_true_1s']:.6g} p_move_zero_1s={summary['p_move_zero_1s']:.6g} p_move_nonmove_1s={summary['p_move_nonmove_1s']:.6g} p_move_move_1s={summary['p_move_move_1s']:.6g} cond_edge_sp_all_1s={summary['cond_edge_sp_all_1s']:.6g} gated_edge_sp_all_1s={summary['gated_edge_sp_all_1s']:.6g} cond_edge_sp_kept_1s={summary['cond_edge_sp_kept_1s']:.6g} gated_edge_sp_kept_1s={summary['gated_edge_sp_kept_1s']:.6g} gated_edge_bal_sign_acc_q50plus_1s={summary['gated_edge_bal_sign_acc_q50plus_1s']:.6g} selection_score={summary['selection_score']}", flush=True)
    ...
    best_val_metrics=best_move_val_metrics
    if best_val_metrics is None: raise ValueError("Missing best validation metrics after move-head selection")
    test_metrics = None
    if LINEAR_STAGE4_RUN_TEST and bool(plan.get("has_cmssl_test", False)):
        ds_test = build_test_dataset_from_plan(plan)
        if ds_test is not None:
            try:
                test_metrics = evaluate_stage4_bundle_streaming(bundle=best_bundle, extractor=extractor, preprocess_bundle=pb, ds=ds_test, stats=stats, device=device, split_name="test", max_rows=LINEAR_STAGE4_MAX_TEST_ROWS)
            finally:
                close_dataset(ds_test, name="stage4_test_eval"); del ds_test; force_gc("stage4_test_eval")
    print(f"[linear-stage4-best] dir_alpha={float(best_direction_result['direction_alpha']):.6g} mag_alpha={float(best_magnitude_result['mag_alpha']):.6g} move_alpha={float(best_move_result['move_alpha']):.6g} auc_1s={float(_metric_at_primary_horizon(best_val_metrics,'dir_auc_kept')):.6g} bal_1s={float(_metric_at_primary_horizon(best_val_metrics,'dir_bal_acc_kept')):.6g} bce_1s={float(_metric_at_primary_horizon(best_val_metrics,'dir_bce_kept')):.6g} mag_huber_1s={float(_metric_at_primary_horizon(best_val_metrics,'mag_primary_huber')):.6g} mag_sp_1s={float(_metric_at_primary_horizon(best_val_metrics,'mag_primary_spearman')):.6g} mag_p50_ratio_1s={float(_metric_at_primary_horizon(best_val_metrics,'mag_primary_p50_ratio')):.6g} mag_p90_ratio_1s={float(_metric_at_primary_horizon(best_val_metrics,'mag_primary_p90_ratio')):.6g} mag_lift_1s={float(_metric_at_primary_horizon(best_val_metrics,'mag_primary_top_bottom_true_mean_lift')):.6g} move_auc_1s={float(_metric_at_primary_horizon(best_val_metrics,'move_auc')):.6g} move_bce_1s={float(_metric_at_primary_horizon(best_val_metrics,'move_bce')):.6g} p_move_zero_1s={float(_metric_at_primary_horizon(best_val_metrics,'move_prob_mean_zero_rows')):.6g} p_move_nonmove_1s={float(_metric_at_primary_horizon(best_val_metrics,'move_prob_mean_nonmove_rows')):.6g} p_move_move_1s={float(_metric_at_primary_horizon(best_val_metrics,'move_prob_mean_move_rows')):.6g} cond_edge_sp_all_1s={float(_metric_at_primary_horizon(best_val_metrics,'cond_edge_spearman_all')):.6g} gated_edge_sp_all_1s={float(_metric_at_primary_horizon(best_val_metrics,'gated_edge_spearman_all')):.6g} cond_edge_sp_kept_1s={float(_metric_at_primary_horizon(best_val_metrics,'cond_edge_spearman_kept')):.6g} gated_edge_sp_kept_1s={float(_metric_at_primary_horizon(best_val_metrics,'gated_edge_spearman_kept')):.6g}", flush=True)
    ...
    stage4_summary_metrics={"primary_horizon_ms": int(PRIMARY_METRIC_HORIZON_MS), "direction": {"auc_kept_1s": float(_metric_at_primary_horizon(best_val_metrics, "dir_auc_kept")), "bal_acc_kept_1s": float(_metric_at_primary_horizon(best_val_metrics, "dir_bal_acc_kept")), "bce_kept_1s": float(_metric_at_primary_horizon(best_val_metrics, "dir_bce_kept"))}, "magnitude": {"mode": "side_cond_log", "log_huber_1s": float(_metric_at_primary_horizon(best_val_metrics, "mag_primary_huber")), "spearman_1s": float(_metric_at_primary_horizon(best_val_metrics, "mag_primary_spearman")), "p50_ratio_1s": float(_metric_at_primary_horizon(best_val_metrics, "mag_primary_p50_ratio")), "p90_ratio_1s": float(_metric_at_primary_horizon(best_val_metrics, "mag_primary_p90_ratio")), "top_bottom_true_mean_lift_1s": float(_metric_at_primary_horizon(best_val_metrics, "mag_primary_top_bottom_true_mean_lift"))}, "move": {"move_auc_1s": float(_metric_at_primary_horizon(best_val_metrics, "move_auc")), "move_bal_acc_1s": float(_metric_at_primary_horizon(best_val_metrics, "move_bal_acc")), "move_bce_1s": float(_metric_at_primary_horizon(best_val_metrics, "move_bce")), "move_pos_frac_true_1s": float(_metric_at_primary_horizon(best_val_metrics, "move_pos_frac_true")), "move_prob_mean_zero_rows_1s": float(_metric_at_primary_horizon(best_val_metrics, "move_prob_mean_zero_rows")), "move_prob_mean_nonmove_rows_1s": float(_metric_at_primary_horizon(best_val_metrics, "move_prob_mean_nonmove_rows")), "move_prob_mean_move_rows_1s": float(_metric_at_primary_horizon(best_val_metrics, "move_prob_mean_move_rows"))}, "edge": {"schema": "p_move_times_conditional_side_edge_v1", "cond_spearman_all": float(_metric_at_primary_horizon(best_val_metrics, "cond_edge_spearman_all")), "cond_spearman_kept": float(_metric_at_primary_horizon(best_val_metrics, "cond_edge_spearman_kept")), "gated_spearman_all": float(_metric_at_primary_horizon(best_val_metrics, "gated_edge_spearman_all")), "gated_spearman_kept": float(_metric_at_primary_horizon(best_val_metrics, "gated_edge_spearman_kept")), "gated_sign_bal_acc_q50plus": float(_metric_at_primary_horizon(best_val_metrics, "gated_edge_bal_sign_acc_q50plus")), "spearman_all": float(_metric_at_primary_horizon(best_val_metrics, "gated_edge_spearman_all")), "spearman_kept": float(_metric_at_primary_horizon(best_val_metrics, "gated_edge_spearman_kept")), "sign_bal_acc_q50plus": float(_metric_at_primary_horizon(best_val_metrics, "gated_edge_bal_sign_acc_q50plus"))}}
    payload={"stage":"stage4","status":"ok","schema":LINEAR_STAGE4_SCHEMA,"stage4_config":cfg,"best_alpha":float(best_direction_result['direction_alpha']),"best_direction_alpha":float(best_direction_result['direction_alpha']),"best_mag_alpha":float(best_magnitude_result['mag_alpha']),"best_move_alpha":float(best_move_result['move_alpha']),"direction_candidate_summaries":direction_summaries,"magnitude_candidate_summaries":magnitude_summaries,"move_candidate_summaries":move_candidate_summaries,"move_head_enabled":True,"move_target_schema":"abs_return_exceeds_side_low_threshold_v1","move_training_rows":"all_decision_rows","move_weighting":LINEAR_STAGE4_MOVE_WEIGHTING,"move_alpha_grid":[float(a) for a in LINEAR_STAGE4_MOVE_ALPHA_VALUES],"final_edge_schema":"p_move_times_conditional_side_edge_v1","best_model_path":str(model_path),"val_metrics":_jsonable_metrics(best_val_metrics),"test_metrics":_jsonable_metrics(test_metrics) if test_metrics is not None else None,"stage4_summary_metrics":stage4_summary_metrics}
