...
def run_stage4_training(*, linear_out_dir: Path, extractor_name: str, preprocess_name: str, device: torch.device) -> Dict[str, Any]:  # type: ignore[override]
    ...
    test_metrics = None
    if LINEAR_STAGE4_RUN_TEST:
        ds_test = build_test_dataset_from_plan(plan)
        try: test_metrics = evaluate_stage4_bundle_streaming(bundle=best_bundle, extractor=extractor, preprocess_bundle=pb, ds=ds_test, stats=stats, device=device, split_name="test", max_rows=LINEAR_STAGE4_MAX_TEST_ROWS)
