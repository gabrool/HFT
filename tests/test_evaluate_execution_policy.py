    ...
    with pytest.raises(FileNotFoundError):
        run_execution_policy_evaluation(
            ExecutionPolicyEvaluationCLIConfig(
                ...
                decision_grid_npz=str(eval_root / "decision_grid.npz"),
                checkpoint_path=str(checkpoint_path),
...
def test_parser_can_disable_l2_trade_dedupe():
    ...
    args = parser.parse_args(["--tape-root", "/tmp/tape", "--decision-grid-npz", "/tmp/tape/decision_grid.npz", "--checkpoint-path", "/tmp/ckpt.pt", "--no-dedupe-l2-decrease-with-trade-prints"])
...
def test_parser_dedupe_l2_trade_default_enabled():
    ...
    parser = build_arg_parser()
    args = parser.parse_args(["--tape-root", "/tmp/tape", "--decision-grid-npz", "/tmp/tape/decision_grid.npz", "--checkpoint-path", "/tmp/ckpt.pt"])
    config = _config_from_args(args)
