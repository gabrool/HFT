...
def _good_snapshot_df() -> pl.DataFrame:
    ...
    return pl.DataFrame([row]).select(list(expected_normalized_columns(TardisDataType.BOOK_SNAPSHOT_25)))
...
def test_snapshot_25_quality_checks():
    df = _good_snapshot_df().with_columns([pl.lit(101.0).alias("bid_px_00"), pl.lit(100.0).alias("ask_px_00"), pl.lit(None).cast(pl.Float64).alias("ask_px_01")])
    ...
    df2 = _good_snapshot_df().with_columns(pl.lit(None).cast(pl.Float64).alias("bid_px_00"))
