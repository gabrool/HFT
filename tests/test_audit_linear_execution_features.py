...
def _write_tape_and_grid(root):
    ...
    save_decision_grid(root / "decision_grid", grid, overwrite=True)
    return grid
...
def test_audit_linear_execution_features_cli_tiny_tape(tmp_path):
    ...
    assert main(["--tape-root", str(root), "--decision-grid", str(root/"decision_grid"), "--linear-train-result-json", str(result), "--overwrite"]) == 0
...
def test_audit_detects_collapsed_no_move_distribution(tmp_path):
    ...
    out=audit_linear_execution_features_from_config(LinearExecutionFeatureAuditConfig(str(root), str(root/"decision_grid"), str(result)))
