...
def test_audit_linear_execution_features_cli_tiny_tape(tmp_path):
    ...
    assert main(["--tape-root", str(root), "--linear-train-result-json", str(result), "--max-decisions", "2", "--overwrite"]) == 0
...
def test_audit_detects_collapsed_no_move_distribution(tmp_path):
    ...
    out=audit_linear_execution_features_from_config(LinearExecutionFeatureAuditConfig(str(root), str(result), max_decisions=2))
